...
class Seating:
    def __init__(self):
        ...
        self.pwm.setPWMFreq(300)  # 50 Hz for analog servo
        ...
        # HEXTRONIK
        self.actuonix = PCA9685(channel=CHANNEL4, freq=300)
        self.actuonix.reset(self.pwm)
    ...
    def controlLoop(self):
        try:
            ...
            while True:
                ...
                dc = int(input("Enter Duty Cycle [30,60]: "))
                uaslog.info("Actuator Position at DC {}".format(dc))
                self.actuonix.setPWM(self.pwm, dutycycle=dc)
                
        except KeyboardInterrupt as e:
            uaslog.warning(f"{e}\nSeating Test Complete.")
            self.cleanup()
            sys.exit(0)
